    ...
    monkeypatch.setattr(
        ...
        lambda: [
            craft_platforms.BuildInfo(
                ...
                build_for="invalid",  # pyright: ignore[reportArgumentType]  # ty:ignore[invalid-argument-type]
    ...
    monkeypatch.setattr(
        ...
        lambda: [
            craft_platforms.BuildInfo(
                platform="something",
                build_on=host_arch,
                build_for=f"{foreign_arch}-{host_arch}",  # pyright: ignore[reportArgumentType]  # ty:ignore[invalid-argument-type]
                build_base=craft_platforms.DistroBase(distro.id(), distro.version()),
            )
        ],
    )
