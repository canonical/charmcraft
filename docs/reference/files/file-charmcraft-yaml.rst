...
.. important::
    ...
    Charmcraft will use the information you provide here to generate :ref:`file-actions-yaml`, :ref:`file-config-yaml`, and :ref:`file-metadata-yaml`, as well as all the other files it usually does.
...
The value of this key is the contents of :ref:`file-actions-yaml`.
...
.. _file-charmcraft-yaml-analysis::
``analysis``
============
...
.. list-table:: Supported features
    ...
    * - ``juju <comparison predicate> <version number>``
      ...
      - Since 2.9.23
    * - ``k8s-api``
      - The charm deploys iff the `backing cloud <https://juju.is/docs/juju/cloud>`_ for the model is Kubernetes.
      - ``k8s-api``
      - Since Juju 2.9.23
...
.. _file-charmcraft-yaml-base::
``base``
========

**Status:** Required in most cases if :ref:`type` is ``charm``.
...
.. _file-charmcraft-yaml-bases:
``bases``
=========

.. note::

    ``bases`` is deprecated, replaced by `base`_, `build-base`_, and `platforms`_.
...
**Status:** Deprecated. Conflicts with the `base`_, `build-base`_, and `platforms`_ keys. Not allowed if `type`_ is ``bundle``.
...
.. _file-charmcraft-yaml-build-base:
``build-base``
==============
...
.. _file-charmcraft-yaml-charm-libs:
``charm-libs``
==============
...
.. _file-charmcraft-yaml-charmhub:
``charmhub``
=====
...
.. _file-charmcraft-yaml-containers:
``containers``
==============
...
.. _file-charmcraft-yaml-description:
``description``
===============
...
.. _file-charmcraft-yaml-extra-bindings:
``extra-bindings``
==================
...
.. warning:: The interface name is the only means of establishing whether two charms are compatible for integration; and carries with it nothing more than a mutual promise that the provider and requirer know the communication protocol implied by the name.
...
.. _file-charmcraft-yaml-resources:
``resources``
=====
...
.. note:: Kubernetes charms must declare an ``oci-image`` resource for each container they define in the `containers`_  mapping.
...
.. _file-charmcraft-yaml-subordinate:
``subordinate``
===============
