...
library, run :ref:`charmcraft publish-lib <ref_commands_publish-lib>`
followed by the full library path on the template ``charms.<charm-name>.v<api-version>.<library-name>``.
For example:
...
In your charm's project file, specify the :ref:`charmcraft-yaml-key-charm-libs` key with the desired
libraries.

In your charm's root directory, run
:ref:`charmcraft fetch-libs <ref_commands_fetch-libs>`.
Charmcraft will download the libraries to your charm's directory.
