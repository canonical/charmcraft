...
def test_upload_pathgiven_home_expanded(tmp_path, config, store_mock):
    """Home-expand the indicated path."""
    fake_home = tmp_path / 'homedir'
    fake_home.mkdir()
    charm_file = fake_home / 'testfile.charm'
    _build_zip_with_yaml(charm_file, 'metadata.yaml', content={'name': 'mycharm'})

    store_mock.upload.return_value = Uploaded(ok=True, status=200, revision=7)
    args = Namespace(filepath=pathlib.Path('~/testfile.charm'))
    with patch.dict(os.environ, {'HOME': str(fake_home)}):
        UploadCommand('group', config).run(args)

    assert store_mock.mock_calls == [
        call.upload('mycharm', charm_file)
    ]


def test_upload_pathgiven_missing(tmp_path, config):
    """The indicated path is not there."""
    missing_path = tmp_path / 'missing.charm'
    args = Namespace(filepath=missing_path)
    with pytest.raises(CommandError) as cm:
        UploadCommand('group', config).run(args)
    assert str(cm.value) == "Cannot access '{}'.".format(missing_path)


def test_upload_pathgiven_not_a_file(tmp_path, config):
    """The indicated path is not a file."""
    args = Namespace(filepath=tmp_path)
    with pytest.raises(CommandError) as cm:
        UploadCommand('group', config).run(args)
    assert str(cm.value) == "'{}' is not a file.".format(tmp_path)
