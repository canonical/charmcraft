...
def test_get_metadata_yaml_reactive_takes_precedence_over_project_file(
    emitter: craft_cli.pytest_plugin.RecordingEmitter,
    package_service,
    service_factory: craft_application.ServiceFactory,
):
    ...
    (dirs.stage_dir / const.METADATA_FILENAME).write_text("name: reactive\n")
    project_dir = service_factory.get("project").resolve_project_file_path().parent
    (project_dir / const.METADATA_FILENAME).write_text("name: project\n")
    service_factory.get("project").get().parts["reactive"] = {"source": "."}
...
def test_needs_packing_when_write_metadata_changed_prime(
    package_service,
    service_factory: craft_application.ServiceFactory,
):
    dirs = service_factory.get("lifecycle").project_info.dirs
...
def test_write_metadata_prefers_project_manifest_yaml(
    package_service,
    service_factory: craft_application.ServiceFactory,
):
    dirs = service_factory.get("lifecycle").project_info.dirs
    ...
    assert (dirs.prime_dir / const.MANIFEST_FILENAME).read_text() == expected


def test_write_metadata_removes_stale_actions_yaml(
    package_service,
    service_factory: craft_application.ServiceFactory,
):
    dirs = service_factory.get("lifecycle").project_info.dirs
    actions_path = dirs.prime_dir / const.JUJU_ACTIONS_FILENAME
    actions_path.parent.mkdir(parents=True, exist_ok=True)
    actions_path.write_text("stale: true\n")
...
def test_write_metadata_removes_stale_config_yaml(
    package_service,
    service_factory: craft_application.ServiceFactory,
):
    dirs = service_factory.get("lifecycle").project_info.dirs
    config_path = dirs.prime_dir / const.JUJU_CONFIG_FILENAME
