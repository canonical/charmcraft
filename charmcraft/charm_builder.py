...
class CharmBuilder:
    ...
    def build_charm(self) -> None:
        ...
        # install user site data from staging venv to charm payload
        if self.python_packages or self.requirements:
        ...
        for basedir, dirnames, filenames in os.walk(
            ...
            for pos, name in enumerate(dirnames):
                ...
                if name == STAGING_VENV_DIRNAME:
                    logger.debug("Ignoring staging venv: %r", str(rel_path))
                    ignored.append(pos)
    ...
    def handle_dependencies(self):
        ...
        if not self.python_packages and not self.requirements:
            return
