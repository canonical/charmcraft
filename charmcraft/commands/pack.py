...
from charmcraft.cmdbase import AppCommand
...
class PackCommand(AppCommand):
    ...
    def _pack_bundle(
        ...
        project: CharmcraftConfig = self.config["services"].project

        if project.parts:
            config_parts = project.parts.copy()
        else:
        ...
        if bundle_part and bundle_part.get("plugin") == "bundle":
            ...
            for fname in MANDATORY_FILES:
                fpath = project.project.dirpath / fname
                if not fpath.exists():
            ...
            if not bundle_part.get("source"):
                bundle_part["source"] = str(project.project.dirpath)
