    ...
    service_factory.get("build_plan").plan = mock.Mock(
        return_value=[
            craft_platforms.BuildInfo(
                ...
                platform="something",
                build_on=HOST_ARCH,
                build_for=plan_build_for,  # pyright: ignore[reportArgumentType]  # ty:ignore[invalid-argument-type]
            )
        ]
    )
    ...
    service_factory.get("build_plan").plan = mock.Mock(
        return_value=[
            craft_platforms.BuildInfo(
                ...
                platform="something",
                build_on=HOST_ARCH,
                build_for=f"{HOST_ARCH}-foreign",  # pyright: ignore[reportArgumentType]  # ty:ignore[invalid-argument-type]
            )
        ]
    )
